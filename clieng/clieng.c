...
typedef struct
{
    ...
    olchar_t ic_strInputBuffer[MAX_COMMAND_LINE_SIZE];
    olchar_t ic_strPrompt[MAX_PROMPT_LEN];
    fnPrintGreeting_t ic_fnPrintGreeting;
    fnPreEnterLoop_t ic_fnPreEnterLoop;
    fnPostExitLoop_t ic_fnPostExitLoop;
    void * ic_pMaster;
    boolean_t  ic_bTerminateClieng;
    boolean_t ic_bEnableScriptEngine;
    olchar_t ic_strInputCmd[MAX_COMMAND_LINE_SIZE];
...
static u32 _createCmdHistory(internal_clieng_t * pic, clieng_param_t * pcp)
{
    ...
    param.cchp_sCmdHistroyBuf = pcp->cp_sCmdHistroyBuf;
    param.cchp_sMaxCmdLine = pcp->cp_sMaxCmdLine;
...
static u32 _createInputOutput(internal_clieng_t * pic, clieng_param_t * pcp)
{
    u32 u32Ret = OLERR_NO_ERROR;
    ...
    param.cip_pstrNewLine = pcp->cp_pstrNewLine;
    param.cip_fhOutput = pcp->cp_fhOutput;
...
static u32 _createParser(internal_clieng_t * pic, clieng_param_t * pcp)
{
    u32 u32Ret = OLERR_NO_ERROR;
    ...
    param.cpp_u32MaxCmdSet = pcp->cp_u32MaxCmdSet;
    param.cpp_pMaster = pcp->cp_pMaster;
...
static u32 _cliengLoop(internal_clieng_t * pic)
{
    ...
    olsize_t length = MAX_COMMAND_LINE_SIZE;
    olchar_t cmd_str[MAX_COMMAND_LINE_SIZE];
    ...
    if (u32Ret == OLERR_NO_ERROR)
    {
    	strcpy(cmd_str, pic->ic_strInputBuffer);
...
u32 clearCliengCommandHistory(void)
{
    u32 u32Ret = OLERR_NO_ERROR;
...
u32 initClieng(clieng_param_t * pParam)
{
    u32 u32Ret = OLERR_NO_ERROR;
    internal_clieng_t * pic = &ls_icClieng;
    ...
    pic->ic_fnPrintGreeting = pParam->cp_fnPrintGreeting;
    pic->ic_fnPreEnterLoop = pParam->cp_fnPreEnterLoop;
    pic->ic_fnPostExitLoop = pParam->cp_fnPostExitLoop;
    pic->ic_pMaster = pParam->cp_pMaster;
    pic->ic_bEnableScriptEngine = pParam->cp_bEnableScriptEngine;
    if (pic->ic_bEnableScriptEngine)
        ol_strncpy(
            pic->ic_strInputCmd, pParam->cp_strInputCmd,
            MAX_COMMAND_LINE_SIZE - 1);
...
u32 cliengNewCmd(
    ...
    void * pParam, fnParseCmd_t fnParseCmd,
    fnProcessCmd_t fnProcessCmd, clieng_cmd_t ** ppCmd)
{
    ...
    u32Ret = newCmd(
        pic->ic_pcpParser, pstrName, fnSetDefaultParam, pParam, 
        fnParseCmd, fnProcessCmd, ppCmd);
...
u32 cliengSetPrompt(const olchar_t * pstrPrompt)
{
    u32 u32Ret = OLERR_NO_ERROR;
    internal_clieng_t * pic = &ls_icClieng;;
