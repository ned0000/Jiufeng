...
typedef struct
{
    ...
    HANDLE id_hDir;
    /**Handle for finding directory entry.*/
    HANDLE id_hFind;
...
static u32 _getFirstDirEntry(jf_dir_t * pDir, jf_dir_entry_t * pEntry)
{
    ...
    {
        ol_bzero(pEntry, sizeof(jf_dir_entry_t));
    ...
    ol_snprintf(
        strDirName, JF_LIMIT_MAX_PATH_LEN - 1, "%s%c*", pid->id_strDirName, PATH_SEPARATOR);
    ...
    if (u32Ret == JF_ERR_NO_ERROR)
    {
        ...
        ol_strncpy(pEntry->jde_strName, FindFileData.cFileName, JF_LIMIT_MAX_PATH_LEN - 1);
    }
#endif
...
/** Ignore the entry from operation system.
 */
static boolean_t _isIgnoreEntry(olchar_t * pstrEntryName)
{
    boolean_t bRet = FALSE;

    if ((strcmp(pstrEntryName, ".") == 0) || (strcmp(pstrEntryName, "..") == 0))
        bRet = TRUE;
    ...
    if (u32Ret == JF_ERR_NO_ERROR)
    {
        ...
        while (u32Ret == JF_ERR_NO_ERROR)
        {
            if (! _isIgnoreEntry(direntry.jde_strName))
            {
                ...
                ol_snprintf(
                    strFullname, sizeof(strFullname) - 1, "%s%c%s", pstrDirName, PATH_SEPARATOR,
                ...
                if (u32Ret == JF_ERR_NO_ERROR)
                {
                    if (jf_file_isDirFile(filestat.jfs_u32Mode))
                    {
                        ...
                        u32Ret = jf_dir_traverse(strFullname, fnHandleFile, pArg);
                    }
                }
            }
...
u32 jf_dir_open(const olchar_t * pstrDirName, jf_dir_t ** ppDir)
{
    ...
    u32Ret = jf_jiukun_allocMemory(&pDir, sizeof(internal_dir_t));

    if (u32Ret == JF_ERR_NO_ERROR)
    {
        ol_bzero(pDir, sizeof(internal_dir_t));

        /*Open the existing directory.*/
        pDir->id_hDir = CreateFile(
            pstrDirName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
            FILE_FLAG_BACKUP_SEMANTICS, NULL);
        if (pDir->id_hDir == INVALID_HANDLE_VALUE)
        {
            u32Ret = JF_ERR_FAIL_OPEN_DIR;
            jf_jiukun_freeMemory(&pDir);
        }
        else
        {
    ...
    jf_dir_fnFilterDirEntry_t fnFilter, jf_dir_fnCompareDirEntry_t fnCompare)
{
    ...
    if (u32Ret == JF_ERR_NO_ERROR)
    {
        ...
        while (u32Ret == JF_ERR_NO_ERROR)
        {
			...
			{
                /*Copy the directory entry to array.*/
				ol_memcpy(start, &direntry, sizeof(jf_dir_entry_t));
				start ++;
				num++;
