...
static u32 _destroyDispatcherXferObject(internal_dispatcher_xfer_object_t ** ppObject)
{
    ...
    jf_logger_logDebugMsg("destroy dispatcher xfer msg obj");
...
static u32 _dispatcherXferObjectRetryConnect(void * object)
{
    ...
    jf_logger_logInfoMsg(
        "dispatcher xfer retry connect, %s, eb %u", str, pidxo->idxo_u32ExponentialBackoff);
        ...
        (olint_t) pow((oldouble_t) 2, (oldouble_t) DISPATCHER_XFER_CONNECT_RETRY_COUNT))
    {
        ...
        jf_logger_logInfoMsg("dispatcher xfer retry connect, rollback to 0");
        pidxo->idxo_u32ExponentialBackoff = 0;
    }

    /*Lets retry again.*/
    jf_logger_logInfoMsg("dispatcher xfer retry connect, try to connect");
...
static u32 _dispatcherXferObjectFreeTimerHandler(void * object)
{
    ...
    jf_logger_logInfoMsg("dispatcher xfer free timer handler");

    if (pidxo->idxo_pdmMsg == NULL)
    {
        /*This connection is idle, because there are no pending message.*/
        jf_logger_logInfoMsg("dispatcher xfer free timer handler, no pending message");
    ...
    jf_network_acsocket_t * pAcsocket, jf_network_asocket_t * pAsocket, u8 * pu8Msg, olsize_t sMsg)
{
    ...
    u32Ret = jf_network_sendAcsocketData(pAcsocket, pAsocket, pu8Msg, sMsg);
...
static u32 _fnDxoEventActionSendData(jf_hsm_event_t * pEvent)
{
    u32 u32Ret = JF_ERR_NO_ERROR;
...
static u32 _dispatcherXferObjectIdleTimerHandler(void * object)
{
    ...
    jf_logger_logInfoMsg("dispatcher xfer idle timer handler");

    if (pidxo->idxo_pdmMsg == NULL)
    {
        jf_logger_logInfoMsg("dispatcher xfer idle timer handler, queue is empty");
        /*This connection is idle, because there are no pending requests */
        /*We need to close this socket.*/
        jf_logger_logInfoMsg("dispatcher xfer idle timer handler, close the connection");
...
static u32 _fnDxoEventActionDataSent(jf_hsm_event_t * pEvent)
{
    ...
    jf_logger_logInfoMsg("dispatcher xfer object action data sent");
...
static u32 _fnDxoEventActionDisconnected(jf_hsm_event_t * pEvent)
{
    ...
    jf_logger_logInfoMsg("dispatcher xfer disconnected event action");
    ...
    if (pidxo->idxo_pdmMsg != NULL)
    {
        ...
        jf_logger_logDebugMsg("dispatcher xfer disconnect, retry later");
...
static u32 _onEntryDxoIdleState(jf_hsm_state_id_t stateId, jf_hsm_event_t * pEvent)
{
    ...
    jf_logger_logDebugMsg("onentry dxo_idle state");
...
static u32 _onExitDxoIdleState(jf_hsm_state_id_t stateId, jf_hsm_event_t * pEvent)
{
    ...
    jf_logger_logDebugMsg("onexit dxo_idle state");
...
static u32 _onEntryDxoInitialState(jf_hsm_state_id_t stateId, jf_hsm_event_t * pEvent)
{
    ...
    jf_logger_logDebugMsg("onentry dxo_initial state");
...
static u32 _onExitDxoInitialState(jf_hsm_state_id_t stateId, jf_hsm_event_t * pEvent)
{
    ...
    jf_logger_logDebugMsg("onexit dxo_initial state");
    ...
    jf_ipaddr_t * pjiRemote, u16 u16Port)
{
    ...
    jf_hsm_transition_t transionTable[] = {
        ...
        {DXOS_CONNECTING, DXOE_CONNECTED, NULL, _fnDxoEventActionSendData, DXOS_OPERATIVE},
        ...
        {DXOS_IDLE, DXOE_SEND_DATA, _isPendingDispatcherXferMsg, _fnDxoEventActionSendData, DXOS_OPERATIVE},
        {JF_HSM_LAST_STATE_ID, JF_HSM_LAST_EVENT_ID, NULL, NULL, JF_HSM_LAST_STATE_ID},
    };

    jf_logger_logInfoMsg("create dispatcher xfer data obj");
    ...
    internal_dispatcher_xfer_object_pool_t * pidxop, dispatcher_msg_t * pdm)
{
    ...
    jf_ipaddr_getStringIpAddrPort(str, &pidxop->idxop_jiRemote, pidxop->idxop_u16RemotePort);
    jf_logger_logInfoMsg("process dispatcher xfer req send data, %s", str);

    if (pidxop->idxop_pidxoMsg == NULL)
    {
        /*There is no previous connection, so we need to set it up*/
        jf_logger_logInfoMsg("process dispatcher xfer req send data, create object");
 ...
 */
static u32 _finishDispatcherXferObjectResponse(internal_dispatcher_xfer_object_t * pidxo)
{
    u32 u32Ret = JF_ERR_NO_ERROR;
    jf_hsm_event_t event;

    jf_logger_logInfoMsg("dispatcher xfer finishd response");
    ...
    u8 * pu8Buffer, olsize_t * psBeginPointer, olsize_t sEndPointer, void * pUser)
{
    ...
    jf_logger_logInfoMsg("dispatcher xfer object data, unexpected data");
    ...
    jf_network_acsocket_t * pAcsocket, jf_network_asocket_t * pAsocket, u32 u32Status, void * pUser)
{
    ...
    {
        jf_logger_logDebugMsg("dispatcher xfer object onconnect, connected");
//        pidxo->idxo_u32ExponentialBackoff = 0;
        ...
        jf_logger_logDebugMsg(
            "dispatcher xfer object onconnect, state: %s",
            _getStringDispatcherXferObjectState(stateId));
    }
    else
    {
        jf_logger_logInfoMsg("dispatcher xfer object onconnect, failed, retry later");
    ...
    jf_network_acsocket_t * pAcsocket, jf_network_asocket_t * pAsocket, u32 u32Status, void * pUser)
{
    ...
    jf_logger_logInfoMsg("dispatcher xfer disconnect");
    ...
    u8 * pu8Buffer, olsize_t sBuf, void * pUser)
{
    ...
    jf_logger_logInfoMsg("dispatcher xfer object send data, status: 0x%X", u32Status);
    ...
    if (u32Status == JF_ERR_NO_ERROR)
    {
        ...
        if (pidxo->idxo_pdmMsg != NULL)
        {
            ...
            /*No response is expected, remove the request.*/
            _finishDispatcherXferObjectResponse(pidxo);
        }
    }
...
u32 destroyDispatcherXferObjectPool(dispatcher_xfer_object_pool_t ** ppPool)
{
    ...
    jf_logger_logDebugMsg("destroy dispatcher xfer object pool");
    ...
    dispatcher_xfer_pool_create_param_t * pdxpcp)
{
    ...
    jf_logger_logDebugMsg("create dispatcher xfer object pool");
