 ...
 *  @file md5.c
 *
 *  @brief The implementation of Message Digest 5
 *
 *  @author Min Zhang
 *
 *  @note
...
static void MD5Update    (MD5_CTX *_ctx, u8 *, u32);
static void MD5Final     (u8 [], MD5_CTX *);
static void MD5Transform (UINT4 [], u8 []);
static void Encode       (u8 *, UINT4 *, u32);
static void Decode       (UINT4 *, u8 *, u32);
    ...
    u32 inputLen)                     /* length of input block */
{
    ...
    if (inputLen >= partLen) {
        memcpy((POINTER)&context->buffer[idx], (POINTER)input, (olsize_t)partLen);
    ...
    memcpy((POINTER)&context->buffer[idx], (POINTER)&input[i], (olsize_t)(inputLen - i));
}
    ...
    MD5_CTX *context)                                       /* context */
{
    ...
    idx = (u32)((context->count[0] >> 3) & 0x3f);
    padLen = (idx < 56) ? ((u32)56 - idx) : ((u32)120 - idx);
...
/* Encodes input (UINT4) into output (u8).
...
/* Decodes input (u8) into output (UINT4).
   Assumes len is a multiple of 4. */
static void Decode(
    UINT4 *output,
    u8 *input,
    ...
    const u8 * pu8Input, u32 u32InputLen, u8 u8Digest[JF_CGHASH_MD5_DIGEST_LEN])
{
    ...
    MD5Update(&md5, (u8 *)pu8Input, u32InputLen);
